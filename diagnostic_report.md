...
Why it works: `compareAndSet` performs the conditional update atomically without blocking threads, providing better scalability under contention.
...
Repeat the original run multiple times to observe nondeterministic final balances. The synchronized version should produce deterministic, consistent final balances per run.
